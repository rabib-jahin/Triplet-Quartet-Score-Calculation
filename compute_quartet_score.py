...
def getSortedTriplet(line, use_weight=True):
    ...
    if use_weight == True:
        return (arr[0], arr[1], arr[2],float(arr[3]))
    else:
...
def get_input_wqrts(input_file_wqrts):
    ...
    with open(input_file_wqrts, mode='r') as fin:
        ...
        for line in lines:
            (t1, t2, t3, t4, w) = getSortedTriplet(line)
            d[(t1, t2, t3, t4)] = w
            total_weight_wqrts += w

    return d, total_weight_wqrts
def get_dictionary_triplets(inputFile):
    os.system("chmod u+x triplets.soda2103")

    dict_triplets = {} # empty dictionary
    tmp_file_name = "TEMP_FILE_PYTHON_FOR_EMBEDDED_QUARTETS"
    file_path = os.path.join("./outputs", inputFile)
    ...
    with open(inputFile) as fin:
        for line in fin: # for each gene tree
            ...
            for line_result in results_array:
                triplet = getSortedTriplet(line_result, use_weight=False)
                
                if line_result not in dict_triplets: # THIS line doesn't exist in dictionary
                    dict_triplets[triplet] = 1 # initialize to 1
                else: # THIS line does exist in dictionary, so increment
                    dict_triplets[triplet] += 1
...
""" Main function """
def main(q_file, stree_file, qscore_level, qscore_output_file=None):
    #dict_quartets, total_weight_wqrts = get_input_wqrts(q_file)
    dict_quartets = get_dictionary_triplets(q_file)
    

    dict_quartets_stree = get_dictionary_triplets(stree_file)
    
  

    satisfied_weights = get_satisfied_weights(dict_quartets, dict_quartets_stree)
    # print(dict_quartets)
    # print(satisfied_weights)

    s = get_string_output(qscore_level, satisfied_weights, 5)
    # print("Triplet score: ",s[0])
    print("Triplet score: ",satisfied_weights)
    ...
    if qscore_output_file != None:
        with open(qscore_output_file, mode='w') as fout:
            fout.write(s)
            fout.write("\n")
    dict_quartets = get_dictionary_quartets(q_file)
    ...
    # s = get_string_output(qscore_level, satisfied_weights, 5)
    # print("Quartet score :",s[0])
    print("Quartet score :",satisfied_weights)
